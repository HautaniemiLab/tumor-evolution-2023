...
#Performs signature attribution for reference signatures on subclonal clusters
#and their supersamples. This is done for each of the SBS, DBS and ID types.
#
#Attribution done in steps:
# 1. Data set wide forward selection for common signatures
# 2. Supersample backward-forward with patient-wise single cluster backward-
#    forward followed by supersample backward (w/ rules) for patient/supersample
#    signatures. Only sufficiently large non-filtered/combined subclones are
#    considered here
# 3. Subclonal cluster backward-selection (w/ rules)
#
#Data set and supersample attributions only consider supersamples or subclones
...
#Name variables
    ATTRIBUTION_SCRIPT="src/attribution/s21_attribute_subclone_signatures.R"

    OUT_DIR="results/attributions/filt_set_$NUM_ID/subclones/reference/"
...
for MUT_TYPE in "SBS" "DBS" "ID"; do
    ...
    IN_FILE_SUPERSAMPLE="data/features/filt_set_${NUM_ID}/subclones/mutational_spectra/supersample_spectra_${MUT_TYPE_LOWER}.tsv"
    IN_FILE_CLUSTERS="data/features/filt_set_${NUM_ID}/subclones/mutational_spectra/subclone_spectra_${MUT_TYPE_LOWER}.tsv"
    ...
    $ATTRIBUTION_SCRIPT -super $IN_FILE_SUPERSAMPLE -cluster $IN_FILE_CLUSTERS \
