...
#R script for attributing signatures on sample tree spectra, given tree branch
#as well as supersample tables. Outputs contributions in both counts as well as
#proportions in a table for supersamples and sample trees each.
...
#      signatures
#   b. single branch level backward-forward selection to get branch signatures
#      to improve sensitivity from combined common and supersample signatures
#      (primary branches only)
#   c. supersample-wise backward selection from combined single tree branch
#      signatures (with rules enforced)
# 3. sample tree branch backward selection (with rules enforced) from
#    supersample signatures to get final signature contributions for individual
#    branches

#Data set and supersample attributions only consider supersamples or tree
#branches respectively with a minimum number of mutations.
...
#Options include: supersample and sample tree branch input spectra table files,
...
# - number of cores to use
    ...
    while (i <= length(args)) {
        ...
        } else if (args[i] %in% c("--tree-branch", "-branch")) {
            #input sample tree branches table
            i = i + 1
            in_file_single_branch = args[i]
    ...
    if (!exists("in_file_supersample") | !exists("in_file_single_branch") | !exists("out_dir") | !exists("mut_class")) {
        stop(
            "Please supply supersample and sample tree branch input spectra table files, mutation class (SBS/DBS/ID) and output directory",
            call. = F
        )
    }
...
#Read the spectra
    ...
    #Single sample
    data_single_branch = read.table(in_file_single_branch, as.is=T, header=T, check.names=F)

    #Strip annotations
    catal_supersample = t(data_supersample[,rownames(ref_sigs), drop=F])
    catal_single_branch = t(data_single_branch[,rownames(ref_sigs), drop=F])
...
#followed by its tree branch backward-forward and
#finally backward (rules enforced) on supersample using combined signatures of its sample tree branches
    #Get patients
    ...
    exps_supersample = mclapply(
        ...
        function(patient) {
            ...
            #Single tree branch backward-forward from supersample signatures
            #Restrict to branches with minimum mutation count of the number of unique mutation types
            branches_of_patient = rownames(data_single_branch)[data_single_branch$patient == patient]
            branches_of_patient = branches_of_patient[
                colSums(catal_single_branch[, branches_of_patient, drop=F]) >= nrow(ref_sigs)
            ]

            #If there are no branches passing mutation count condition, return the supersample exposure
            if (length(branches_of_patient) == 0) return(patient_fit_orig)

            branch_fit_orig = lapply(
                branches_of_patient,
                function(branch) {
                    fitSigBF(
                        catal_single_branch[, branch, drop=F], ref_sigs,
                        unique(c(
            ...
            #Join branch exposures
            exps_joined = joinSigExposures(branch_fit_orig, ref_sigs)

            #Supersample backward with rules from joined branch signatures
            exps_out = fitSigBF(
...
# --------------------------------------------------
# Sample tree branch backward selection
# --------------------------------------------------

#Attribution for each sample tree branch backward from its respective supersample's signatures
    #Fit each patient's tree branches
    exps_branches_by_patient = mclapply(
        patients,
        function(patient) {
            #Branches of the patient
            branches_of_patient = rownames(data_single_branch)[data_single_branch$patient == patient]
            branches_of_patient = branches_of_patient[
                colSums(catal_single_branch[, branches_of_patient, drop=F]) > 0
            ]

            #Single tree branch backward with rules from supersample signatures
            exps_by_branch = lapply(
                branches_of_patient,
                function(branch) {
                    #Limit forward selection in sensitive mode to branches with minimum
                    #mutation count of the number of unique mutation types
                    fwd_thres = ifelse(sum(catal_single_branch[,branch]) >= nrow(ref_sigs), mdl$thres_block_fwd, Inf)

                    fitSigBF(
                        catal_single_branch[, branch, drop=F], ref_sigs, rownames(exps_supersample[[patient]]),
                        forced_sigs = sigs_rules_forced, connected_sigs = sigs_rules_conn,
            ...
            #Join branch exposures
            exps_out = joinSigExposures(exps_by_branch, ref_sigs)

            #Remove 0-contribution signatures
            exps_out[rowSums(exps_out) > 0,, drop=F]
        }
    )
    names(exps_branches_by_patient) = patients

#Join sample tree branch exposures
    exps_single_branch_joined = joinSigExposures(exps_branches_by_patient, ref_sigs)
...
#Format and write output tables with mutation count and accuracy (1 - cosine similarity)
    ...
    #Sample tree branches, original frequencies
    writeTable(
        prepareOutputTable(exps_single_branch_joined, catal_single_branch, F, data_single_branch),
        paste0(out_dir, "contributions_", tolower(mut_class), "_sample_tree_branch.tsv")
    )
    #Sample tree branches, proportions
    writeTable(
        prepareOutputTable(exps_single_branch_joined, catal_single_branch, T, data_single_branch),
        paste0(out_dir, "contributions_", tolower(mut_class), "_sample_tree_branch_proportions.tsv")
    )
