...
#R script for attributing signatures on subclonal spectra, given subclonal
#cluster as well as supersample tables. Outputs contributions in both counts as
#well as proportions in a table for supersamples and subclones each.
...
#      signatures
#   b. single subclone level backward-forward selection to get subclonal
#      signatures to improve sensitivity from combined common and supersample
#      signatures (selected subclones only)
#   c. supersample-wise backward selection from combined single subclone
#      signatures (with rules enforced)
# 3. subclonal cluster backward selection (with rules enforced) from
#    supersample signatures to get final signature contributions for individual
#    subclones

#Data set and supersample attributions only consider supersamples or subclonal
#clusters respectively with a minimum number of mutations.
...
#Options include: supersample and subclonal cluster input spectra table files,
...
# - number of cores to use
    ...
    while (i <= length(args)) {
        ...
        } else if (args[i] %in% c("--subclonal-cluster", "-cluster")) {
            #input subclonal cluster table
            i = i + 1
            in_file_single_cluster = args[i]
    ...
    if (
        !exists("in_file_supersample") | !exists("in_file_single_cluster") |
        !exists("out_dir") | !exists("mut_class")
    ) {
        stop(
            "Please supply supersample and subclonal cluster input spectra files, mutation class (SBS/DBS/ID) and output directory",
            call. = F
        )
    }
...
#Read the spectra
    ...
    #Single cluster
    data_single_cluster = read.table(in_file_single_cluster, as.is=T, header=T, check.names=F)

    #Strip annotations
    catal_supersample = t(data_supersample[,rownames(ref_sigs), drop=F])
    catal_single_cluster = t(data_single_cluster[,rownames(ref_sigs), drop=F])
...
# --------------------------------------------------
# Prepare list of supersample subclones
# --------------------------------------------------

subclones_influential_list = sapply(
    colnames(catal_supersample),
    function(patient) {
        rownames(data_single_cluster)[
            data_single_cluster$patient == patient &
            ! data_single_cluster$filtered &
            ! data_single_cluster$combination
        ]
    },
    simplify = F
)
...
#followed by its subclonal cluster (select subclones) backward-forward and
#finally backward (rules enforced) on supersample using combined signatures of its subclones
    #Get patients
    ...
    exps_supersample = mclapply(
        ...
        function(patient) {
            ...
            #Single subclone (specified in subclone list) backward-forward from supersample signatures
            #Restrict to clusters with minimum mutation count of the number of unique mutation types
            clusters_of_patient = subclones_influential_list[[patient]]
            clusters_of_patient = clusters_of_patient[
                colSums(catal_single_cluster[, clusters_of_patient, drop=F]) >= nrow(ref_sigs)
            ]

            #If there are no clusters passing mutation count condition, return the supersample exposure
            if (length(clusters_of_patient) == 0) return(patient_fit_orig)

            cluster_fit_orig = lapply(
                clusters_of_patient,
                function(cluster) {
                    fitSigBF(
                        catal_single_cluster[, cluster, drop=F], ref_sigs,
                        unique(c(
            ...
            #Join cluster exposures
            exps_joined = joinSigExposures(cluster_fit_orig, ref_sigs)

            #Supersample backward with rules from joined cluster signatures
            exps_out = fitSigBF(
...
# --------------------------------------------------
# Subclonal cluster backward selection
# --------------------------------------------------

#Attribution for each subclonal cluster backward from its respective supersample's signatures
    #Fit each patient's subclonal clusters
    exps_clusters_by_patient = mclapply(
        patients,
        function(patient) {
            #Clusters of the patient
            clusters_of_patient = rownames(data_single_cluster)[data_single_cluster$patient == patient]
            clusters_of_patient = clusters_of_patient[
                colSums(catal_single_cluster[, clusters_of_patient, drop=F]) > 0
            ]

            #Subclonal cluster backward with rules from supersample signatures
            exps_by_cluster = lapply(
                clusters_of_patient,
                function(cluster) {
                    #Limit forward selection in sensitive mode to clusters with minimum
                    #mutation count of the number of unique mutation types
                    fwd_thres = ifelse(sum(catal_single_cluster[,cluster]) >= nrow(ref_sigs), mdl$thres_block_fwd, Inf)

                    fitSigBF(
                        catal_single_cluster[, cluster, drop=F], ref_sigs, rownames(exps_supersample[[patient]]),
                        forced_sigs = sigs_rules_forced, connected_sigs = sigs_rules_conn,
                        bwd_thres = mdl$thres_bwd, fwd_thres=fwd_thres, bwd_stop_n=1,
                        f_sel_stat = mdl$f_sel_stat, f_stop_stat = mdl$f_stop_stat,
                        method = mdl$method
                    )
                }
            ...
            #Join cluster exposures
            exps_out = joinSigExposures(exps_by_cluster, ref_sigs)

            #Remove 0-contribution signatures
            exps_out[rowSums(exps_out) > 0,, drop=F]
        }
    )
    names(exps_clusters_by_patient) = patients

#Join subclonal cluster exposures
    exps_single_cluster_joined = joinSigExposures(exps_clusters_by_patient, ref_sigs)
...
#Format and write output tables with mutation count and accuracy (1 - cosine similarity)
    ...
    #Subclonal clusters, original frequencies
    writeTable(
        prepareOutputTable(exps_single_cluster_joined, catal_single_cluster, F, data_single_cluster),
        paste0(out_dir, "contributions_", tolower(mut_class), "_subclone.tsv")
    )
    #Subclonal clusters, proportions
    writeTable(
        prepareOutputTable(exps_single_cluster_joined, catal_single_cluster, T, data_single_cluster),
        paste0(out_dir, "contributions_", tolower(mut_class), "_subclone_proportions.tsv")
    )
