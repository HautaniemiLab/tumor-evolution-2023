...
#profiles for SBS, DBS and ID mutations of both supersamples (select subclones)
#and subclonal clusters (and combinations of them).
...
#Print used options
    ...
    if [[ $ZERO_INDEXED_CLUSTERS != "" ]]; then
        echo "- Assuming zero-indexed clusters for drawing subclonal exposure plots"
    fi
    echo ""

#Signature attribution
    echo "Attributing signatures"
    src/attribution/s20_attribute_subclone_ref_sigs.sh -n $NUM_ID $REGEX_DATASET_SELECTION \
...
#Plot signature exposures and fits
    ...
    src/visualisation/s80_draw_subclone_sig_attributions.sh -n $NUM_ID $REFERENCE_SIGNATURES \
        $ZERO_INDEXED_CLUSTERS
